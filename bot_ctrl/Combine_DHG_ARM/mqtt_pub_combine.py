...
global stiffness_value, set_point_value, dhg_device
stiffness_value=0.0 
set_point_value=0.0
...
###########################################3 MQTT Block #####################################
# MQTT Settings
BROKER = "54.89.231.201" #"10.10.7.199"     # Change to IP if remote
PORT = 1883
...
def on_message(client, userdata, msg):
    ...
    if msg.topic==TOPIC_SUB:
        print("updating stiffness {}".format(data["xela_1"]))
        print("updating set_point {}".format(data["arm_shape"]))
        logging.info("updating stiffness {}".format(data["xela_1"]))
        logging.info("updating set_point {}".format(data["arm_shape"]))
        call_stiff_changer(data["xela_1"], data["arm_shape"])
